...
function buildPath(file, isScheme) {
    return fs.existsSync(file)
      ? file
      : toRelativePath(isScheme
        ? `db/schemes/${file}.yml`
        : `db/templates/${file}.nunjucks`)
}
...
Examples:
  ...
  const templPath = buildPath(templName, false);
  const schemePath = buildPath(schemeName, true);
