...
(async function() {
  ...
  const {template: templNameOrPath, scheme: schemeNameOrPath, shade} = cli.flags;
  ...
  if (!templPath) {
    ...
    templPath = path.join(__dirname, `db/templates/${templNameOrPath}/${shade}.nunjucks`);
  }
  const schemePath = path.join(__dirname, `db/schemes/${schemeNameOrPath}.yml`);

  let templ;
  try {
    templ = await fs.readFile(templPath, 'utf8');
  } catch (err) {
    console.error(`Could not find a template called ${templNameOrPath} in the database.`);
    return;
  }
  ...
  try {
    ...
    console.error(`Could not find a scheme called ${schemeNameOrPath} in the database.`);
    return;
  }
